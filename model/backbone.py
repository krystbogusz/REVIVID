...
1. **Restoration + SR** — all T frames are degraded LR; backbone propagates
   features bidirectionally and produces a coarse HR restoration.
2. **Spatial inpainting** — persistent holes are baked into the LQ video by the
...
class ConditioningBackbone(nn.Module):
    def __init__(
        ...
        self.trunk = make_layer(ResidualBlockNoBN, 4, num_feat=num_feat * 2)
        ...
        self.hr_blocks = make_layer(ResidualBlockNoBN, 2, num_feat=num_feat * 2)
    ...
    def _comp_flow(self, lrs: torch.Tensor):
    ...
    def forward(self, lrs: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        fwd_flow, bwd_flow = self._comp_flow(lrs)
        ...
        for i in range(t - 1, -1, -1):
            if i < t - 1:
                ...
                warped_pix = flow_warp(lrs[:, i + 1], flow)
                res_ind = torch.abs(
                    rgb_to_luma(warped_pix) - rgb_to_luma(lrs[:, i])
                )
        ...
        for i in range(t):
            ...
            if self.sr_scale > 1:
                curr_hr = F.interpolate(
                    lrs[:, i], size=hr_size, mode="bilinear", align_corners=False
                )
            else:
                curr_hr = lrs[:, i]

            coarse = torch.tanh(self.coarse_head(fused_hr) + curr_hr)
